	...
	public function getRatesByProductId($product_id, $customer_id)
	{
		$sql   = "
			  SELECT
				rc.*,
				rc.review_id as cid,
                IF ((SELECT urc.delta  FROM " . DB_PREFIX . "rate_review urc WHERE urc.customer_id = '" . (int)$customer_id . "' AND urc.review_id = rc.review_id)  < 0, -1 ,  IF ((SELECT urc.delta  FROM " . DB_PREFIX . "rate_review urc WHERE urc.customer_id = '" . (int)$customer_id . "' AND urc.review_id = rc.review_id)  > 0, 1 ,  0 ) ) as customer_delta ,
